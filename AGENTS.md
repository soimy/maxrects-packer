...
- 发布内容由 `package.json` 的 `files` allowlist 决定，且**刻意与 2.7.x 对齐**：`dist` + `src` + `assets/{custom.css,custom.js,favicon.ico}` + `CHANGELOG.md` + `tsconfig*.json` + `typedoc.json` + `UPGRADE_SUMMARY.md`（实测 tarball 62.3kB / 31 文件，2.7.4 是 61.9kB / 33 文件）。与 2.7.4 的差异只有：不再发布已删除的 `eslint.config.js` / `.eslintrc.json` 和 CI 配置 `.github/workflows/node.js.yml`，新增 `dist/maxrects-packer.cjs`。**没有 `.npmignore`**，npm 会回落到 `.gitignore`（里面含 `dist`/`lib`），但 `files` 优先，dist 仍会进包。注意 `assets/*.png` 不在 allowlist 里（2.7.x 也没发），所以 README 图片在 npm 页面依旧不显示；把 `assets` 整目录加进去可修复，代价是 tarball 从 ~62kB 涨到 ~502kB。`clean` 同时删 `dist` 与历史遗留的 `lib`。
