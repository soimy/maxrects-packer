...
class MaxRectsBin extends abstract_bin_1.Bin {
    constructor(maxWidth = maxrects_packer_1.EDGE_MAX_VALUE, maxHeight = maxrects_packer_1.EDGE_MAX_VALUE, padding = 0, options = { smart: true, pot: true, square: true, allowRotation: false }) {
        ...
        this.freeRects.push(new Rectangle_1.Rectangle(this.maxWidth + this.padding, this.maxHeight + this.padding));
    ...
    add(...args) {
        ...
        if (node) {
            ...
            if (!rect) {
                rect = new Rectangle_1.Rectangle(width, height, node.x, node.y, node.rot);
                rect.data = data;
            }
            else {
                rect.x = node.x;
            ...
            if (this.updateBinSize(new Rectangle_1.Rectangle(width + this.padding, height + this.padding, this.width + this.padding, 0))
                || this.updateBinSize(new Rectangle_1.Rectangle(width + this.padding, height + this.padding, 0, this.height + this.padding))) {
                return rect ? this.add(rect) : this.add(width, height, data);
            }
        }
        else {
            if (this.updateBinSize(new Rectangle_1.Rectangle(width + this.padding, height + this.padding, 0, this.height + this.padding)) || this.updateBinSize(new Rectangle_1.Rectangle(width + this.padding, height + this.padding, this.width + this.padding, 0))) {
                return rect ? this.add(rect) : this.add(width, height, data);
            }
        }
        return undefined;
    ...
    findNode(width, height) {
        ...
        for (let i in this.freeRects) {
            ...
            if (r.width >= height && r.height >= width) {
                ...
                if (areaFit < score) {
                    bestNode = new Rectangle_1.Rectangle(height, width, r.x, r.y, true); // Rotated node
                    score = areaFit;
                }
            }
        }
    ...
    splitNode(freeRect, usedNode) {
        ...
        if (usedNode.x < freeRect.x + freeRect.width && usedNode.x + usedNode.width > freeRect.x) {
            ...
            if (usedNode.y > freeRect.y && usedNode.y < freeRect.y + freeRect.height) {
                let newNode = new Rectangle_1.Rectangle(freeRect.width, usedNode.y - freeRect.y, freeRect.x, freeRect.y);
                this.freeRects.push(newNode);
            }
            // New node at the bottom side of the used node
            if (usedNode.y + usedNode.height < freeRect.y + freeRect.height) {
                let newNode = new Rectangle_1.Rectangle(freeRect.width, freeRect.y + freeRect.height - (usedNode.y + usedNode.height), freeRect.x, usedNode.y + usedNode.height);
                this.freeRects.push(newNode);
            }
        }
        // Do Horizontal split
        if (usedNode.y < freeRect.y + freeRect.height &&
            ...
            if (usedNode.x > freeRect.x && usedNode.x < freeRect.x + freeRect.width) {
                let newNode = new Rectangle_1.Rectangle(usedNode.x - freeRect.x, freeRect.height, freeRect.x, freeRect.y);
                this.freeRects.push(newNode);
            }
            // New node at the right side of the used node.
            if (usedNode.x + usedNode.width < freeRect.x + freeRect.width) {
                let newNode = new Rectangle_1.Rectangle(freeRect.x + freeRect.width - (usedNode.x + usedNode.width), freeRect.height, usedNode.x + usedNode.width, freeRect.y);
                this.freeRects.push(newNode);
            }
        }
        return true;
    ...
    expandFreeRects(width, height) {
        ...
        this.freeRects.push(new Rectangle_1.Rectangle(width - this.width - this.padding, height, this.width + this.padding, 0));
        this.freeRects.push(new Rectangle_1.Rectangle(width, height - this.height - this.padding, 0, this.height + this.padding));
        this.freeRects.forEach((freeRect, index) => {
