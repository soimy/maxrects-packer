...
class MaxRectsBin extends abstract_bin_1.Bin {
    ...
    findNode(width, height) {
        ...
        let areaFit;
        let r;
        let bestNode = new Rectangle_1.Rectangle();
    ...
    updateBinSize(node) {
        ...
        if (!new Rectangle_1.Rectangle(0, 0, this.width, this.height).contain(node))
            return false;
        let tmpWidth = Math.max(this.maxWidth, node.x + node.width - this.padding);
        let tmpHeight = Math.max(this.maxHeight, node.y + node.height - this.padding);
        if (this.options.pot) {
